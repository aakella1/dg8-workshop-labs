...
public class ScoreService {
    ...
    /*
     * public void save(Score entry) { scoreCache.put(getKey(entry), entry, 5,
     * TimeUnit.SECONDS); }
     */
    ...
    // simple
    /*
     * void onStart(@Observes @Priority(value = 1) StartupEvent ev){ cacheManager =
     * new DefaultCacheManager(); ConfigurationBuilder config = new
     * ConfigurationBuilder(); //config.expiration().lifespan(5, TimeUnit.SECONDS);
     * //config.memory().size(2).build();
     * cacheManager.defineConfiguration("scoreboard", config.build()); scoreCache =
     * cacheManager.getCache("scoreboard"); scoreCache.addListener(new
     * CacheListener());
     * 
    ...
    void onStart(@Observes @Priority(value = 1) StartupEvent ev) {
        ...
        //distributed sync
        //config.expiration().lifespan(5, TimeUnit.MINUTES).clustering().cacheMode(CacheMode.DIST_SYNC);
        //replicated sync
